...
if (isset($_POST['buscar_horario'])) {
    ...
    $sql = "SELECT nombre, apellido, altura, numero_de_telefono_jugador, mail_del_jugador, peso, edad, habilidad, disponibilidad_horaria_jugador, sub, zona_residencial FROM formulario_jugadores WHERE posicion_jugador = 'delantero centro' AND disponibilidad_horaria_jugador LIKE '%$horario%'";
...
if (isset($_POST['buscar_zona'])) {
    $zona = $_POST['zona'];
    $sql = "SELECT nombre, apellido, altura, numero_de_telefono_jugador, mail_del_jugador, peso, edad, habilidad, disponibilidad_horaria_jugador, sub, zona_residencial FROM formulario_jugadores WHERE posicion_jugador = 'delantero centro' AND zona_residencial LIKE '%$zona%'";
    $result = $conn->query($sql);
}

// Si no se ha realizado ninguna búsqueda, mostrar todos los jugadores
if (!isset($_POST['buscar_horario']) && !isset($_POST['buscar_zona'])) {
    $sql = "SELECT nombre, apellido, altura, numero_de_telefono_jugador, mail_del_jugador, peso, edad, habilidad, disponibilidad_horaria_jugador, sub, zona_residencial FROM formulario_jugadores WHERE posicion_jugador = 'delantero centro'";
    $result = $conn->query($sql);
}
?>
...
<body>
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<p class='no-players-message'>No se encontraron jugadores de Delantero.</p>";
    }
    ?>
</body>
